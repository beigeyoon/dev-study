...
- **모바일 반응형 추가** — 컨테이너 폭 ≤640px에서 헤더 2줄 분리·검색창 풀폭·터치 타겟 42px·패널 풀스크린·범례 축소. (아래 "Responsive" 섹션)
- **폰트 = Wanted Sans** — UI·본문·노드 라벨에 적용. 코드 블록만 mono 유지. (Pretendard 아님)
...
- 폰트(변경됨):
  - **UI·본문·노드 라벨 = Wanted Sans** (가변폰트). CDN: `https://cdn.jsdelivr.net/gh/wanteddev/wanted-sans@v1.0.3/packages/wanted-sans/fonts/webfonts/variable/split/WantedSansVariable.min.css`. CSS `font-family: 'Wanted Sans', 'Wanted Sans Variable', system-ui, sans-serif`. 코드베이스에 이미 Wanted Sans가 번들돼 있으면 그걸 사용.
  - **코드(`.kg-body code`) = mono 유지**: `ui-monospace, SFMono-Regular, Menlo, monospace`.
  - ⚠️ Pretendard는 더 이상 쓰지 않는다(이전 버전 잔재 — 교체 완료).
- 타입 스케일: 11(라벨/캡션) · 12(배지/통계) · 13(검색/버튼/도메인) · 14(본문) · 15(타이틀) · 21(패널 제목).
- 트랜지션: 0.15s(요소) / 0.24s ease(패널).

## Responsive (모바일)
프로토타입은 **컨테이너 폭 기준**(`ResizeObserver`로 루트 폭 관측, ≤640px = 모바일)으로 전환한다 — 뷰포트 미디어쿼리가 아니라, 임베드 시에도 동작하게 하기 위함. 코드베이스에서는 동일 동작을 `@media (max-width:640px)` 또는 컨테이너 쿼리 중 환경에 맞는 쪽으로 구현하면 된다. 데스크톱 레이아웃·인코딩·색은 변경 없음.

모바일(≤640px)에서 바뀌는 것:
- **헤더 2줄 분리** — 타이틀+컨트롤(빈틈만·줌)이 1줄, **검색창이 풀폭으로 2번째 줄**. 헤더 높이 고정 해제(`height:auto`), `flex-wrap:wrap`, padding `9px 12px`.
- **통계 텍스트 숨김** (`{{ stats }}`) + 헤더 우측 스페이서 제거.
- **터치 타겟 42px** — 줌 버튼·"빈틈만" 버튼·검색 입력 높이 모두 34→42px.
- **"빈틈만" 라벨 텍스트 숨김** — 점선 아이콘만 표시(버튼 자체는 유지).
- **상세 패널 풀스크린** — `width:100%`(데스크톱은 `min(440px,92%)`).
- **범례 축소** — `max-width:150px`, padding 축소, 인코딩 키 3줄(크기/채움/빈틈 설명) 숨김. 도메인 필터 행은 유지.

프로토타입 구현 참고(클래스 토글 방식): 루트에 `kg-root`, 모바일일 때 `kg-root mobile` 클래스를 부여하고 `.kg-root.mobile .kg-*` 규칙으로 위 값들을 `!important`로 덮어쓴다. 타깃 클래스: `.kg-header / .kg-stats / .kg-spacer / .kg-search / .kg-gapbtn / .kg-gap-label / .kg-tbtn(줌버튼) / .kg-legend / .kg-legend-keys / .kg-panel`.

## Assets
- **Cytoscape.js 3.30.2** — 그래프 렌더링 엔진 (기존 의존성). 프로토타입은 `cdn.jsdelivr.net`에서 로드(원본은 cdnjs).
- **Wanted Sans** (가변 웹폰트) — UI/본문/라벨용. CDN(jsdelivr) 또는 코드베이스 번들 사용. `https://github.com/wanteddev/wanted-sans` (OFL 라이선스).
