...
- [ ] **인덱스 / B-tree** — (importance 4, **database 다음 칸 1순위.** 🔄 **범위 축소 2026-07-31:** "인덱스가 뭘 파는가"(공간·쓰기 속도)와 "왜 이기는가"(접근 횟수 250,000배)는 복습에서 보강 완료. **남은 것 = B-tree 구조 자체** — 500만 → 20번이 정확히 어떻게 되나. 단 7-31에 *"인덱싱이 되어 있어서"* 동어반복이 나왔으므로, **8-03 복습에서 인덱스 결함이 해소된 뒤**에 여는 게 순서상 맞음)
