...
- ✅ **database 2번째 점(2026-08-04): [B-tree](../knowledge/concepts/b-tree.md) 생성**(learning, m1). *"인덱스는 어떻게 3번 만에 닿나"* 미해결 질문을 여는 페이지. **창고 안내판 비유까지 도달** — 블록·fanout·분할은 미소화로 페이지에 "다음 층"으로 격리 보관. database 2개(평균 2.0).
  - 🔴 **신규 관찰 — 코치의 투입량 실패.** 1차 시도에 16KB·fanout 400·log·split·해시/스킵리스트를 한 번에 넣어 *"너무 어려워서 못 알아들었어"*. 전부 버리고 **그림 하나**로 줄이자 즉시 붙음. → **규율: 새 개념 1회차엔 그림 하나만, 숫자·용어는 그림이 선 뒤.**
- 🔴 **1차 원리 3세션 연속 자력 실패(2026-08-04) — 결론: 진단은 유지, 방법은 폐기.**
  - 7-31에 세운 진단(**"암묵적 사용 ↔ 명시적 언어화의 간극"**)은 8-04에도 재확인됐다. 오늘 세 지점 전부 같은 모양이었다 — 메커니즘 ✅ / 이름 붙이기 ❌:
    | | 메커니즘 | 명명 |
    |---|---|---|
    | 1차 원리 | ✅ *"어떻게 가격이 MySQL 수준이죠?"* (공짜는 없다를 이미 씀) | ❌ 힌트 2단계까지 |
    | 청구서 ③ | ✅ *"익스포트해서 거기서 검색"* | ❌ "일의 주인이 바뀐다"까지 못 감 |
    | 인덱스 | ✅ *"원본과 인덱스 둘 다 수정"* | ❌ *"인덱싱이 안 돼서"* |
  - 🎯 **진단 정밀화:** 말로 못 꺼내는 게 아니라 **이름을 아는 것이 설명을 밀어낸다.** 라벨이 있으면 "답했다"고 착각하고 멈춘다.
  - 🔄 **그러나 방법은 폐기.** 3세션 연속 *"그 문장을 말해봐"* 로 실패했다 = 방법이 틀렸다. **앞으로 문장을 요구하지 않는다** — 원리 없이는 못 푸는 상황만 던지고 **결정만 받는다.** 문장이 없어도 판단이 맞으면 통과.
  - ⚠️ **동시에, 이 결함의 크기를 과대평가하지 말 것.** 8-04에 사용자가 지적한 대로 코치가 **정답 문구를 정해놓고 맞추게 한 것**이 실패의 일부였다. 명시적 언어화가 m5의 유일한 관문인지 자체를 재검토 대상으로 둔다.
...
- **database 내 다음 칸:** ~~B-tree 인덱스 구조~~(✅ 8-04 착수) / 캐시 무효화 실전(delete vs update vs TTL, write-through) / Redis 영속화(RDB·AOF) / 캐시 miss 폭주(스탬피드) / 트랜잭션·ACID. **단 B-tree가 m3 이상으로 굳기 전엔 database 신규를 더 열지 않는다**(8-04에 m1로 겨우 그림 하나 세운 상태).
