...
### 4.1 세션 예산제 (공백 내성 — 목표 주 2–3회)
사용자는 매일 세션을 열 수 없다(일이 바쁨). 공백은 SRS의 적이 아니다(간격 효과는 오히려 공백을 원한다). 폭탄일의 진짜 원인은 *공백*이 아니라 **복귀 시 몰아치기**다. 따라서:
- **due는 마감이 아니라 우선순위 큐다.** 세션마다 due 전부가 아니라 **상위 3–4개 `model` 유닛만** 처리(통합 앵커 1방 = 여러 개념 묶은 1유닛). `reference`는 가벼운 존재 점검이라 예산 무관.
- **우선순위 = importance × 얼마나 밀렸나.** imp5 밀린 것이 최상위. 나머지는 죄책감 없이 다음 세션으로 미룬다(코치가 먼저 "나머지는 다음에"라고 안심시킬 것).
- **공백은 벌하지 않는다.** 오래 밀렸어도 무힌트 통과 시 오히려 `review_due`를 **한 단계 더 점프**시킨다(durable 증거). 못 꺼낸 것만 1일 리셋.
- **부하 높으면 신규 동결.** 세션 예산을 복습이 다 먹었거나 2주+ 밀린 항목이 있으면 그 세션엔 신규 ingest 보류.
- **폭탄일 금지.** "11개 due"가 떠도 상위 3–4개만. 복귀 시 절대 다 갈려 하지 말 것 — 몰아치기가 방전→이월→재폭탄의 진짜 원인이다.
